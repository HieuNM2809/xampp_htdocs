...
( function( $ ) {
	...
	function _econis_update_total_wishlist(){
	...
	function _econis_accordion_menu(){	
	...
	function _headercategories(){
		...
		$('.econis-close',$menu_categories).on( "click", function() {
			$menu_categories.removeClass('active');
			return false;
		});		
		//Navigation right			
	...
	function _canvasrightNavigation(){
		...
		$('.econis-close',$wpbingo_menu_right).on( "click", function() {
			$wpbingo_menu_right.removeClass('active');
			return false;
		});		
		//Navigation right			
		...
		$(".input-search",$element).on("keydown", function() {
			...
			if(character.length >= 2){
				...
				$.ajax({
					...
					dataType: 'json',
					data: {
						action : "econis_search_products_ajax",
						character : character,
						limit : limit,
						category : category
					},
	...
	function _load_360_view_product(){
		...
		$(".econis-360-button").on( "click", function() {
	...
	function _click_button(){
		...
		$(".econis-bt-shipping").on( "click", function() {
	...
	function _after_add_to_cart(){
		$( _body ).on( 'added_to_cart', function(){
			_click_add_to_cart();
			$('body').append('<div class="econis-product-added"><div class="added-message">'+$(".mini-cart").data("text_added")+'</div>');
			setTimeout(function(){
				$('.econis-product-added').remove();
			},3000);
		});
	}
	function _click_add_to_cart(){
		$(".cart-remove").on( "click", function() {
			if( $(".econis-topcart.popup").hasClass("active") ){
				$(".econis-topcart.popup").removeClass("active");
			}
		...
		$(".cart-icon").on( "click", function() {
			if( !_body.hasClass("not-scroll") && $(".econis-topcart").hasClass("popup") ){
	...
	function _click_quickview_button(){
		$('.quickview-button').on( "click", function(e) {
			...
			$.ajax({
				url: econis_ajax_url,
				data: {
					"action" : "econis_quickviewproduct",
	...
	class Elementor_Js_Econis {
		static getInstance() {
			if (!Elementor_Js_Econis.instance) {
				Elementor_Js_Econis.instance = new Elementor_Js_Econis();
			}
			return Elementor_Js_Econis.instance;
		}
		constructor() {
	...
	Elementor_Js_Econis.getInstance();	
} )( jQuery );